...
def Create_Yolo(input_size=416, channels=3, training=False, classes=["component"], anchors = []):
    ...
    for i, conv_tensor in enumerate(conv_tensors):
        pred_tensor = decode(conv_tensor, anchors,NUM_CLASS, i)
        if training:
...
def decode(conv_output, anchors ,NUM_CLASS, i=0):
...
def compute_loss(pred, conv, label, bboxes, i=0, classes=["component"]):
