...
def Load_Yolo_model():
    ...
    # if len(gpus) > 0:
    #     print(f'GPUs {gpus}')
    #     try: tf.config.set_memory_growth(gpus[0], True)
...
def detect_image(Yolo, image_path, output_path, input_size=416, show=False, classes={}, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    ...
    while True:
        if Predicted_data.qsize() > 0:
            ...
            image = draw_bbox(original_image, bboxes,
                              classes, rectangle_colors=rectangle_colors)
...
def detect_video_realtime_mp(video_path, output_path, input_size=416, show=False, CLASSES={}, score_threshold=0.3, iou_threshold=0.45, rectangle_colors='', realtime=False):
    if realtime:
...
def detect_video(Yolo, video_path, output_path, input_size=416, show=False, classes={}, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    times, times_2 = [], []
    vid = cv2.VideoCapture(video_path)
...
def detect_realtime(Yolo, output_path, input_size=416, show=False, classes={}, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    times = []
    vid = cv2.VideoCapture(0)
    ...
    while True:
        ...
        frame = draw_bbox(original_frame, bboxes, classes,
                          rectangle_colors=rectangle_colors)
