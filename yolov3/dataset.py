...
class Dataset(object):
    ...
    def __init__(self, dataset_type, config, TEST_INPUT_SIZE=TEST_INPUT_SIZE):
        self.annot_path = config["data"]["train"] if dataset_type == 'train' else config["data"]["test"]
        ...
        self.anchors = (np.array(config["model"]["anchors"]).T/self.strides).T
    ...
    def Delete_bad_annotation(self, bad_annotation):
        ...
        bad_image_name = bad_annotation[0].split(
            '/')[-1]  # can be used to delete bad image
        ...
        with tf.device('/cpu:0'):
            ...
            if self.train_yolo_tiny:
                batch_label_mbbox = np.zeros(
                    (self.batch_size, self.train_output_sizes[0], self.train_output_sizes[0], self.anchor_per_scale, 5 + self.num_classes), dtype=np.float32)
                batch_label_lbbox = np.zeros(
                    (self.batch_size, self.train_output_sizes[1], self.train_output_sizes[1], self.anchor_per_scale, 5 + self.num_classes), dtype=np.float32)
            else:
                ...
                batch_label_mbbox = np.zeros(
                    (self.batch_size, self.train_output_sizes[1], self.train_output_sizes[1], self.anchor_per_scale, 5 + self.num_classes), dtype=np.float32)
                batch_label_lbbox = np.zeros(
                    (self.batch_size, self.train_output_sizes[2], self.train_output_sizes[2], self.anchor_per_scale, 5 + self.num_classes), dtype=np.float32)
            ...
            if self.batch_count < self.num_batchs:
                while num < self.batch_size:
                    ...
                    try:
                        if self.train_yolo_tiny:
                            label_mbbox, label_lbbox, mbboxes, lbboxes = self.preprocess_true_boxes(
                                bboxes)
                        else:
                        ...
                        print("IndexError, something wrong with",
                              annotation[0], "removed this line from annotation file")
