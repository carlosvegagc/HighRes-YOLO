...
def train(config):
    ...
    trainset = Dataset('train', config)
    testset = Dataset('test', config)
    ...
    for epoch in range(TRAIN_EPOCHS):
        ...
        if len(testset) == 0:
            ...
            yolo.save_weights(os.path.join(
                TRAIN_CHECKPOINTS_FOLDER, config["model"]["name"]))
            continue
        ...
        if TRAIN_SAVE_CHECKPOINT and not TRAIN_SAVE_BEST_ONLY:
            save_directory = os.path.join(
                TRAIN_CHECKPOINTS_FOLDER, config["model"]["name"]+"_val_loss_{:7.2f}".format(total_val/count))
