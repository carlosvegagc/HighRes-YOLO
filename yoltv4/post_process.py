...
from yoltv4.prep_train import convert_poly_coords
            ...
            for box, score, classy in zip(boxes_bin, scores_bin, classes_bin):
                ...
                if score >= plot_thresh:
                    ...
                    if draw_circle:
                        ...
                        cv2.circle(overlay, (cx, cy), r, color,
                                   plot_line_thickness, lineType=2)
        ...
        for box, score, classy in zip(boxes, scores, classes):
            ...
            if score >= plot_thresh:
                ...
                if draw_circle:
                    ...
                    r = int(d/2.0)
                    cx, cy = int((left+right)/2.0), int((top+bottom)/2.0)
                    cv2.circle(output, (cx, cy), r, color,
                               plot_line_thickness, lineType=2)
    ...
    if len(gt_bounds) > 0:
        ...
        for gt_bound in gt_bounds:
            ...
            if draw_circle:
                ...
                r = int(d/2.0)
                cx, cy = int((left+right)/2.0), int((top+bottom)/2.0)
                cv2.circle(output, (cx, cy), r, gt_color,
                           plot_line_thickness_gt, lineType=4)

            # plot categories too (on bottom)
            if show_labels:
                # adapted from visuatlizion_utils.py
                # or classy, whch is '1 = airplane'
                display_str = 'gt: ' + str(gt_cat)
                # If the total height of the display strings added to the top of the bounding
                # box exceeds the top of the image, stack the strings below the bounding box
                # instead of above.
                #display_str_heights = [font.getsize(ds)[1] for ds in display_str_list]
...
def execute(pred_dir='/root/darknet/results/',
    ...
    for i, im_name in enumerate(im_names_tiled):
        ...
        outfile_geojson_geo = os.path.join(
            geojson_geo_dir, im_name + '.geojson')
        outfile_geojson_pix = os.path.join(
            geojson_pix_dir, im_name + '.geojson')
        ...
        if im_name not in im_names_set:
            ...
            df_tmp2 = pd.DataFrame(
                {'category': [''], 'prob': [0], 'geometry': [geom_tmp2]})
        ...
        # else, get all boxes for this image, create a list of box names too
            ...
            # make and save gdf
            gdf_geo = gpd.GeoDataFrame(geom_list_geo, columns=[
                                       'geometry', 'category', 'prob'], crs=crs)
            gdf_geo.to_file(outfile_geojson_geo, driver='GeoJSON')
            gdf_pix = gpd.GeoDataFrame(geom_list_pix, columns=[
                                       'geometry', 'category', 'prob'], crs=crs)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--detection_thresh', type=float, default=0.16,
                        help="yolo threshold")
    parser.add_argument('--nms_overlap_thresh', type=float, default=0.4,
