...
def create_mask(im_path, label_path, out_path_mask, burnValue=255):
    with fiona.open(label_path, "r") as annotation_collection:
        annotations = [feature["geometry"]
                       for feature in annotation_collection]
...
def get_window_geoms(df, window_size=416, jitter_frac=0.2, image_w=0, image_h=0,
    ...
    for index, row in df.iterrows():
        ...
        for k in range(n_wins):
            ...
            geom_window = shapely.geometry.Polygon(
...
def yolt_from_df(im_path, df_polys,
    ...
    if verbose:
        ...
        print("  object count:",
              df_im_tmp[category_col].value_counts().to_dict())
        ...
        for i, window_geom in enumerate(window_geoms_aug):
            ...
            # just apply a rotation to every third
                ...
                for row in obj_list:
                    [index_nest, cat_nest, x0, y0, x1, y1] = row
                    yolt_row = [cat_nest] + \
                        list(convert((w, h), [x0, x1, y0, y1]))
                    # cat_idx = cat_idx_dic[cat_nest]
                    # yolt_row = [cat_idx, cat_nest] + list(convert.convert((w,h), [x0,x1,y0,y1]))
                    yolt_coords.append(yolt_row)
                df_yc = pd.DataFrame(yolt_coords, columns=[
                                     'cat', 'x', 'y', 'w', 'h'])
                ...
                image_outfile = os.path.join(
                    outdir_ims_aug, outroot + yolt_image_ext)
                label_outfile = os.path.join(
                    outdir_labels_aug, outroot + '.txt')
                # save
                skimage.io.imsave(image_outfile, win_out)
                df_out.to_csv(label_outfile, sep=' ',
...
def get_labels(csv_path, xmin, ymin, width, height, label_col, min_overlap=0, classes=True):
    ...
    frac_overlap = tile_gdf.geometry.apply(
        lambda x: tile_bbox.intersection(x).area/x.area)
    ...
    tile_gdf.geometry = tile_gdf.geometry.apply(
        lambda x: tile_bbox.intersection(x))
    tile_gdf.geometry = tile_gdf.geometry.apply(
    ...
    geom_bounds = np.around(
        geom_bounds[['midx', 'midy', 'width', 'height']].to_numpy(), 6).tolist()
    if classes:
...
def main():
    ...
    for i, subdir in enumerate(subdirs):
        ...
        # (assume pan is the same size as ps-rgb will be!)
    ...
    for im_dir_tmp in [
        ...
        sample_label_vis_dir = os.path.join(os.path.dirname(
            im_dir_tmp), 'sample_label_vis_dir' + suff)
